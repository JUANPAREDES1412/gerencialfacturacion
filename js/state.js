...
/**
 * Diagnóstico: detecta filas que son EXACTAMENTE iguales en todos sus campos
 * clave (n_admi, f_admi, v_admi, n_fact, n_ate01, sede). Dos líneas legítimas
 * de una misma admisión casi siempre difieren en valor o tipo de atención —
 * si son idénticas en todo, lo más probable es que un archivo se haya
 * cargado dos veces (por ejemplo, si el borrado previo falló). Devuelve un
 * resumen por sede para mostrarlo en "Cargar información".
 */
APP.detectarDuplicadosExactos = function () {
  const vistos = new Map();
  const porSede = {};
  APP.state.registros.forEach(r => {
    const key = [r.sede, r.n_admi, r.f_admi, r.v_admi, r.n_fact, r.n_ate01].join("||");
    const cnt = (vistos.get(key) || 0) + 1;
    vistos.set(key, cnt);
    if (cnt === 2) { // se cuenta como "grupo duplicado" la primera vez que se repite
      porSede[r.sede] = (porSede[r.sede] || 0) + 1;
    }
  });
  return Object.entries(porSede).map(([sedeId, gruposDuplicados]) => ({
    sede: APP.getSede(sedeId) || { id: sedeId, nombre: sedeId },
    gruposDuplicados
  }));
};
