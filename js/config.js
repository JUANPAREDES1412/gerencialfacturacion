...
/* ----------------------------------------------------------------------------
   LOGIN (opcional)
   Si está en true, la app exige iniciar sesión (requiere haber corrido el
   bloque de autenticación de supabase/schema.sql). Si está en false, la app
   entra directo, sin pedir usuario ni clave — pero solo funciona si además
   corriste supabase/desactivar_login.sql (ver README), porque si no, las
   reglas de seguridad de la base de datos seguirán exigiendo sesión y la
   app no podrá leer ni cargar datos.
   ---------------------------------------------------------------------------- */
APP.REQUERIR_LOGIN = false;
