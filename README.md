...
**Para activarla:**
1. Corre el bloque de políticas "solo autenticados / solo admin" de `supabase/schema.sql` (ya está en ese archivo).
2. Crea tus administradores como se explica más abajo.
3. En `js/config.js`, pon `APP.REQUERIR_LOGIN = true;` y sube el cambio a GitHub.

**Para desactivarla de nuevo más adelante** (sin perder los usuarios ya creados):
1. Corre `supabase/desactivar_login.sql` (vuelve el acceso a la base de datos a abierto).
2. En `js/config.js`, pon `APP.REQUERIR_LOGIN = false;` y sube el cambio a GitHub.
