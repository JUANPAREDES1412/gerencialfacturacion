...
/** Acceso público al mismo cliente (compartido con auth.js, para que la
 *  sesión de login se aplique también a las consultas de datos). */
APP.obtenerClienteSupabase = clienteSupabase;
...
APP.guardarDatosSedeRemoto = async function (sedeId, clinicaId, registros, infoArchivo, onProgreso) {
  ...
  // 1.1 Verificación de seguridad: confirmar que en verdad ya no quedan filas
  // de esta sede antes de insertar las nuevas. Si por un problema de permisos
  // el borrado no tuvo efecto, aquí se detiene el proceso en vez de sumar los
  // datos nuevos ENCIMA de los viejos (lo que duplicaría/infla los valores).
  const { count: restantes, error: errConteo } = await sb
    .from(TABLA_REGISTROS)
    .select("id", { count: "exact", head: true })
    .eq("sede", sedeId);
  if (errConteo) throw new Error("No se pudo verificar el borrado previo: " + errConteo.message);
  if (restantes && restantes > 0) {
    throw new Error(
      `No se pudo completar la limpieza de datos previos de la sede (aún quedan ${restantes} filas). ` +
      "Por seguridad NO se cargó el archivo nuevo, para evitar duplicar información. " +
      "Verifica que tu usuario tenga rol de Administrador y vuelve a intentarlo."
    );
  }
